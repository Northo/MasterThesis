...
\label{sec:ll-notilt}
Cuple the Weyl Hamiltonian to the magnetic field through minimal coupling
\begin{equation}
  \label{eq:weyl-hamil}
...
Introduce the \(\vec{B}\)-field by the minimal coupling \(\vec{p} \to \vec{p}^B = \vec{p} + e \vec{A}\).
We take the field to be in the \( z \)-direction, and use the Landau gague \(\vec{A} = -B_{z}y \hat{x}\).
\todo{Before applying the temperature gradient, when we still only consider finding the LLs, we may in fact say that we have generally \( t_{\perp} \) only in x, and hen later rotate into coordinates where \( \nabla T \) is in y. Maybe that is interesting after all...?}
...
We may again make the plane wave ansatz \( \phi(\vec{r}) = e^{i k_x x + i k_z z} \phi(y) \), similar to what was done for the untilted Hamiltonian in section \ref{sec:ll-notilt}, to replace \( p_{(x /y)} \to k_{(x /y)} \).
...
\begin{equation}
  \label{eq:46}
  E = t^s_{\parallel} v_{F} \tilde{k}_{z} + \sign(m) v_F \alpha \sqrt{2 |m| e B \alpha  + \tilde{k}_{z}^2}.
\end{equation}
