...
First, let us consider the exponent of the product.
...
\begin{summary}
  ...
  \begin{align}
    ...
    J_{\vec{k} m s; \vec{k} + \qvec{q} n s} (\vec{q}) &=
                                                        ...
                                                        \begin{multlined}[t]
                                                          \Big[ \alpha_{k_z m s} \Xi_{1} (\vec{q}, m, n, s)\\
                                                            + \alpha_{k_z + \qvec{q}_z n s} \Xi _{2} (\vec{q}, m, n, s) \Big],
                                                        \end{multlined}\\
    T_{\vec{k} + \qvec{q} n s, \vec{k} m s}^{0y} (\vec{q}) &=
                                                             \frac{is \alpha}{2}
                                                             \Gamma ^+_{\vec{k} \qvec{q} m n s}
...
However, in the untilted case, the sum was restricted to only one quadrant, as at \( T\to 0 \) the transitions must be between states with energies of opposite signs.
...
In the untilted system, the contributions from the two chiralities were the same.
...
The contribution \eqref{eq:125} is odd in \( t_z \), and so for systems with broken inversion symmetry, the total contribution from two cones cancels.
...
Here we have also reinserted the explicit \( \hbar \).
...
\begin{figure}[p]
  ...
  \caption{Contributions to \( \gamma_{\bar{N}} \) from \( m\to n \) transitions for different values of \( t_x \).
    ...
    % Note also that the \( \gamma_{\bar{N}} \) square regions indicated in the first pane are drawn with lengths \( 2 \bar{N} + 1 \), while they in the main text are defined with sides \( 2 \bar{N} \);
    ...
    % Note that the squares indicating \( \gamma_{\bar{N}} \) have sides of length \( 2 \bar{N} + 1 \), while they are defined in the main text as having length \( 2 \bar{N} \);
    % this choice is only for clarity in the figure, as the colored tiles have edges at half-integer values.
    % The squares indicating \( \gamma_{\bar{N}} \) are drawn with sides 1 larger than their definitions; this is to avoid confusion, as the colored tiles have edges at half-integer values.
    The squares indicating \( \gamma_{\bar{N}} \) are drawn with sides 1 larger than their definitions, as the colored tiles have edges at half-integer values.
    \label{fig:contribs}}
\end{figure}
...
The explicit form of the contribution was found analytically, given in \cref{eq:124,eq:125}.
The contribution from each new Landau level, \( \gamma_{\text{div}, \bar{N}}-\gamma_{\text{div}, \bar{N}} \) is shown in \cref{fig:divergent-factor} for some of the lowest Landau levels.
In the large cutoff limit, \( \Lambda \gg 1 \), expanding and dropping terms \( \mathcal{O}(1 /\Lambda^2) \), we find,
\begin{equation}
 \label{eq:126}
  \gamma_{\text{div}, \bar{N}} - \gamma_{\text{div}, \bar{N}-1} =
  t_z
...
We also investigated the untitled part, \( \gamma_{\bar{N}}^0 \), under the momentum cutoff.
We found the integral over momentum to converge very quickly, and so the momentum has little effect.
The result found for no cutoff is therefore used.
    ...
    \begin{tikzpicture}
      ...
      \begin{axis}[
        ...
        cycle multiindex* list={
          ...
          solid\\ dashdotted\\ dashed\\ loosely dashed\\ dashdotdotted\\
        },
        % cycle multiindex* list={
        %   color\nextlist
        %   solid\\ dashed\\ dashdotted\\ loosely dashed\\
        % }
        ]
      ...
      \end{axis}
    \end{tikzpicture}
    % \caption{The divergent factor \( \gamma_{\text{div}, \bar{N}} / t_{z} \) for the first Landau levels, as a function of the momentum cutoff \( \Lambda \).
    %   Shown is the addition of each new level \( ( \gamma_{\text{div}, \bar{N}} - \gamma_{\text{div}, \bar{N}} ) / t_{z} \).
    % % \caption{The divergent factor \( ( \gamma_{\text{div}, \bar{N}} - \gamma_{\text{div}, \bar{N}} ) / t_{z} \) for the first Landau levels, as a function of the momentum cutoff \( \Lambda \).
    %   Each level has the same \( \Lambda \) dependence;
    %   they are separated by a \( \Lambda \) independent factor, and the separation decreases for higher levels.
    %   See main text for details.
    % }
    %
    \caption{The addition of each new Landau level to the divergent factor \( \gamma_{\text{div}, \bar{N}} / t_{z} \), as a function of the momentum cutoff \( \Lambda \).
      Each level has the same \( \Lambda \) dependence;
      they are separated by a \( \Lambda \) independent factor, and the separation decreases for higher levels.
      See main text for details.
...
However, in inversion symmetric systems also the odd component contributes.
