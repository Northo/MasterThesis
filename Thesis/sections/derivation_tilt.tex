...
Lastly, notice that in the static limit, the dependence on \( k_z \) disappears, and so the same procedure as was done for the untilted cone in section \ref{sec:response_notilt} is valid for the tilted cone, replacing the \( \vec{k} \) sum with an integral over \( k_z \) and a degeneracy factor
\begin{equation}
  \label{eq:103}
...
\begin{equation}
  \label{eq:138}
  \lim_{\omega \to 0} \lim_{\vec{q} \to 0} \chi^{xy} = \frac{e^2v_F B}{(2\pi)^2} (\gamma_N^0 + \gamma_{\text{div}, N}),
\end{equation}
where \( \gamma_N^0 \) is the prefactor of the untilted case, and
\begin{equation}
  \label{eq:97}
  \gamma_{\text{div}, N} = -2 \sum\limits_{i=0}^{N} \int \mathrm{d}\kappa_z \xi(\kappa_z) \kappa_z t^s_z \alpha_{\kappa_z m s}^2 \Big|_{\overset{m=i+1}{n=-i}},
\end{equation}
...
\begin{equation}
  \label{eq:139}
  \gamma_{\text{div}, 0} = \frac{t_z}{2} \left(\Lambda  \left(\Lambda -\sqrt{\Lambda ^2+1}\right)+\sinh ^{-1}(\Lambda
   )\right)
\end{equation}
...
\begin{multline}
  ...
  \frac{t_z}{2}
  \Bigg\{
    \Lambda\left(\sqrt{\Lambda^2 + m - 1} - \sqrt{\Lambda^2 + m}  \right)\\
    + m \tanh^{-1}\left[\frac{\Lambda}{\sqrt{\Lambda^2 + m} } \right]
    - (m - 1) \tanh^{-1}\left[\frac{\Lambda}{\sqrt{\Lambda^2 + m - 1}}\right]
    \Bigg\},
    \label{eq:98}
\end{multline}
...
\begin{equation}
  ...
  \left(
    \frac{1}{4}\left[
  -1 - m \log \left(1 - \frac{1}{m}\right) + \log \left(\frac{m - 1}{4}\right)
  \right]
 + \frac{\log(\Lambda)}{2}
...
\right  ).
\end{equation}
For a general \( n \to m, \; N>0, M=N+1 \) transition, the contribution was found to have very lengthy expressions.
Consult Table~\ref{tab:typeii-expressions} to find the expressions for positive and negative tilt, and interband and intraband transitions.
...
\begin{table}[ht]
  ...
  \caption{
    ...
    Decision matrix over different regions for the expression of the \( m \to n; N > 0, M =N+1 \) transition, given in code listing of the corresponding number.
    Listings starting on page \pageref{lst:typeii-interband-tzpos} in \cref{cha:long_expression}.
...
As described above, the contribution from the cone with chirality \( s=-1 \) is for inversion symmetric systems the same as that of \( s=+1 \) at opposite tilt \( t_z \to -t_z \).
...
For some contribution \( \gamma(t_z) \), we define
\begin{align}
  \gamma_{\text{even}}(t_z) &= \frac{\gamma(t_z) + \gamma(-t_z)}{2}\label{eq:132},\\
  \gamma_{\text{odd}}(t_z) &= \frac{\gamma(t_z) - \gamma(-t_z)}{2}\label{eq:133}.\\
\end{align}
...
\subsection{Perpendicular tilt}
\todo{Make sure to include negative tilt also}
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}
    ...
    \begin{axis}[
      ...
      \addplot+[mark=none] table[col sep=comma, y=1] {data/contribtypeii_intraband_odd.csv};
      ...
      \addplot+[mark=none] table[col sep=comma, y=4] {data/contribtypeii_intraband_odd.csv};
      ...
      \addplot+[mark=none, dashed] table[col sep=comma, y=4] {data/contribtypeii_intraband_even.csv};
    \end{axis}
  \end{tikzpicture}
  } %% Subcaptionbox
...
In literature, however, it is sometimes written that Type-II break Lorentz symmetry,
\todo{cite}
...
% However, consider again
% \begin{equation}
%   \label{eq:130}
%   v_F \vec{\sigma} \vec{p} \ket{\psi} = E \ket{\psi}.
% \end{equation}
% Lorentz boosting in the \( x \)-direction
% \begin{equation*}
%   \begin{split}
%     \tilde{p}_x &= \gamma (p_x - \beta E),\\
%     \tilde{E} &= \gamma (E - \betae p_x).
%   \end{split}
% \end{equation*}
% Eq. \eqref{eq:130} thus reads
% \begin{equation}
%   \label{eq:131}
%   v_F (\sigma^x \gamma (p_x - \beta E) + \sigma^yp_y \sigma^zp_z) = \gamma (E - \beta p_x).
% \end{equation}
% If we now rescale
% \[
%   \begin{split}
