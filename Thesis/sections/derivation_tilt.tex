...
For perpendicular tilt, as there are contributions also away from the \( M=N + 1 \) line, we denote by \( \gamma_0 \) the contributions from inside the square of length 3 centered at the origin.
The \( \gamma_1 \) contributions are those inside the square of length 5, and in general \( \gamma_n \) the square with length \( 2 n + 1 \).
...
\begin{figure}[ht]
  ...
  \end{tikzpicture}
  \caption{Total contribution \( \gamma_N \) for a perpendicular tilt \( t_x \).
...
\subsubsection{Perpendicular tilt with only zeroth level transitions}
...
However, there are also other possible ways one may consider including the Landau level cutoff.
