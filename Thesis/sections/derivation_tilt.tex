...
In the Type-I regime, the contributions differ from that of the untitled system by \( \gamma_{\text{div}, N} \), Eq. \eqref{eq:98}, dependent on a momentum cutoff \( \Lambda \).
...
Considering firstly only the lowest Landau level contribution, Eq. \eqref{eq:117},, which is odd in \( t_z \), the total contribution cancel between the chiralitites for broken inversion symmetry, while it adds up for inversion symmetric systems.
