...
Recalling the expression for \(\tilde{\phi}\) from Eq. \eqref{eq:51},
\[
  \tilde{\phi} = e^{-\frac{1}{2} \chi ^2}
  \begin{pmatrix}
    a_{\vec{k} m s} H_{M-1} (\chi)\\
...
\begin{figure}[ht]
  ...
  \tikzsetnextfilename{contribmn-Pbroken}
...
Using now the effective speed of light \( c=v_F \), Eq. \eqref{eq:137} is
\begin{equation}
  \label{eq:138}
  - v_F^2 p_{\mu } p^{\mu }.
\end{equation}
