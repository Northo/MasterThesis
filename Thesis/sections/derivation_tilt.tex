...
The contribution is odd in \( t_z \), so for systems with broken inversion symmetry, the two chiralities the two cones cancel, and the response is equal to the untilted case.
...
As \( |t_z| -> 1 \) from above, the contribution blows up.
...
The contribution goes to zero as \( t_z \to \infty \), shown in figure \ref{fig:01contribtzII}.
\begin{figure}[ht]
  \centering
  \tikzsetnextfilename{01tzcontribtypeii}
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{The contribution from the \(0 \to 1\) transition in a Type-II \(t_z\) tilted system.}
  \label{fig:01contribtzII}
\end{figure}
