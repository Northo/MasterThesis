...
The contribution from each new Landau level, \( \gamma_{\text{div}, N}-\gamma_{\text{div}, N} \) is shown in \cref{fig:divergent-factor}.
...
The cutoff \( N \) is not tunable in any obvious way -- we have already assumed the system to be in the deep quantum limit where only the zeroth level is Landau level is filled.
...
Recall that the contribution is \( \gamma^0_N + \gamma_{\text{div}, N} \).
