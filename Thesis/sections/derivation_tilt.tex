...
The integral was computed numerically for \( M,N \leq 14 \) over different values of \( t_x \) with \( t_z = 0 \), with the individual contributions shown in figure~\ref{fig:contribs}.
...
The total contribution \( \gamma_{\bar{N}} \) as a function of \( \bar{N} \) is shown in figure~\ref{fig:total_contribs}.
...
This is especially pertinent in light of the non-physical behaviour seen around the Lifshitz transition;
in the transition from Type-I to Type-II the chiral anomaly is not broken, and chiral anomaly effects have been found to have analytical behaviour in the transition, even for the linear model~\cite{sharmaChiralAnomalyLongitudinal2017}.
...
If we define by \( \gamma_{\bar{N}} \) the sum of all \( 0\to n, |n| < \bar{N} \) transitions, and compute \( \gamma_{\bar{N}} \) as a function of the tilt \( t_x \) for various \( \bar{N} \), we get the result shown in figure~\ref{fig:0tontx}.
