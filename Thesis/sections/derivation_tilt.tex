...
The integrand in the response is the same as the untitled case, with the exception of the factor \( \epsilon_{\kappa_z m s} + \epsilon_{\kappa_z n s} = \epsilon_{\kappa_z m s}^0 + \epsilon_{\kappa_z n s}^0 + 2\kappa_z t_z^s \).
The interesting behavior thus comes from the \( 2 \kappa_z t_z^s \) term;
in the inversion symmetric case, \( \vec{t}^s = s \vec{t} \), and the contributions from the two chiralitites are the same as we once again have \( \kappa_z s \).
In the case of broken inversion symmetry, however, \( \vec{t}^s = \vec{t} \), and \( \kappa_z \) appears without the \( s \) factor, meaning that under \( (\kappa_z, s) \mapsto (-\kappa_z, -s) \), the term changes sign.
Thus, in the case of broken inversion symmetry, the terms cancel between the chiralities, and the response is the same as for the untilted system.
For inversion symmetric systems, however, there is an additional contribution from the \( 2 \kappa_z t_z^s \) term, which we will see is of great importance.
...
The \( 0\to m \) transitions where computed analytically, and found to be
\begin{equation}
  \label{eq:117}
  \frac{1}{2}
  \left(
...
\right  ),
\end{equation}
interestingly independent of \( m \).
