...
Alternatively, the same result is found by simply inserting the definition Eq. (\ref{eq:13}) for both $\vec{E}$ and $\sigma $ in Eq. (\ref{eq:ohm}), and use
\[
...
\end{equation}
By the Fourier transformation introduced in Eq. (\ref{eq:13})
\[
  A(\vec{q}, \omega ) =
  \iint \mathrm{d}t \mathrm{d} \vec{r}
  e^{i(\omega  t - \vec{q} \vec{r} )}
...
To do this, firstly the $00$-element of the stress-energy tensor will be expressed in terms derivatives of $T^{0j}$, and then a partial integration will swap the derivative between the stress-energy tensor and gravitational potential.
Note first that in the flat system the conservation law of the  energy and momentum is simply
\begin{equation}
  \partial _0 T^{00}(t, \vec{r}) + v_F \partial _i T^{0i} (t, \vec{r}) = 0,
\end{equation}
...
\end{equation}
Introduce Eq. (\ref{eq:16}) in the response relation (\ref{eq:current-from-gravity}), and use integration by parts
\begin{equation}
  \int u v' = uv - \int u' v,
\end{equation}
giving
...
\end{equation}
where care must be taken to distinguish the stress-energy tensor $T^{0j}$ and the temperature $T$, differentiated by the indices, or lack thereof.
