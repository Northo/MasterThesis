...
Taking $H=H_F$, the part of the Hamiltonian whose dynamics we are interested in, and integrating over the interaction time, the result is reminiscent of \cref{eq:lin-response-1}.
...
\todo{Consider including some of Mahan's discussion about having a thermal gradient when we in the calculations have assumed constant temperature.}%
...
Earlier calculations of the transport properties of temperature gradients were conducted from local variable theories;
...
Instead, one introduces the fictitious field $\psi$, for which the origin is not addressed, and finds the transport coefficients for this system.
...
\begin{figure}[h]
  ...
  \caption{Illustration of Luttinger's solution to heat transport. To include a temperature fluctuation $T$, couple the system to some (possibly fictious) gravitational potential $\psi$ giving the same current response as the temperature fluctuation.}
...
This is discussed more with regard to the results of this thesis in \cref{sec:transport-magnetization}.
...
the reason for choosing other definitions might be to have more convenient expressions for the Onsager relation, Seebeck coefficient, thermal conductivity tensor, etc.~\cite{mahanManyparticlePhysics2000,chernodubThermalTransportGeometry2021,lundgrenThermoelectricPropertiesWeyl2014}.
...
To do this, firstly the $00$-element of the energy-momentum tensor will be expressed in terms of derivatives of $T^{j0}$, and then a partial integration will swap the derivative between the energy-momentum tensor and gravitational potential.
Note first that in the flat system the conservation law of the  energy and momentum is simply%
\footnote{We used the conservation law of the energy-momentum tensor \( \partial_{\mu}T^{\mu\nu}=0 \), and \( \partial_0 = \partial_t /v_F \).}
\begin{equation}
  \partial _t T^{00}(t, \vec{r}) + v_F \partial _i T^{i0} (t, \vec{r}) = 0,
\end{equation}
