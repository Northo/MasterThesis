...
\todo{Dirac and Weyl cones are the emergence of non-gapped linear energy bands in condensed matter physics, in effect exhibiting relativistic behavior at non-relativistic speeds.
...
This model, however, fails for Dirac materials, where the electrons behave as ``Dirac fermions''.
...
In condensed matter language, however, we mean by fermion that it obeys the Pauli exclusion principle and follows the Fermi-Dirac distribution function.
...
Thus, the theory and results form high energy physics may be applied in these emergent Dirac systems.
...
Unfortunately, some concepts of QFT and high energy physics are somewhat inaccessible for condensed matter physicists.
At the same time, the condensed matter description have been difficult to relate back to the QFT formalism.
...
The recent and excellent review paper by \textcite{chernodubThermalTransportGeometry2021} attempts to make the topics approachable for researchers from both fields, whose ``main purpose [...] is to present the basic notions underlying new developments in condensed matter in a language equally accessible to both high energy and condensed matter communities''.
...
\end{equation}
where \( \slashed{\partial} = \gamma^{\mu} \partial_{\mu} \), \( \gamma^{\mu} \) the gamma matrices \footnote{Also known as the Dirac matrices. They are any irreducible matrix representation of the Clifford algebra.}, \( m \) is some mass parameter, and \( \psi \) the Dirac spinor.
The systems is \( 4\times 4 \).
...
In some older literature, the term might also be applied to 2D systems, sucha as Graphene.
...
\end{equation}
\todo{Could we comment that this corresponds to re-constructing the Dirac equation from the two chiral Weyl equations, and \emph{then} addihng the lorentz breaking terms?}
...
If one instead perturbs the system with a Zeeman field in the $x$-direction, i.e. having a $b' > 0$, the separation is instead in energy, giving nodal loop where the two cones intersect.
...
The Hamiltonian in Eq. (\ref{eq:3}) is not the most general, if we allow for anisotropy in the system.
In three dimensions we have more generally the Hamiltonian
\begin{equation}
  \label{eq:4}
  H = \vec{v}_0 \vec{p} + (\vec{v} \odot \vec{p}) \vec{\sigma} ,
\end{equation}
where $\vec{v}_0$ is  the \emph{tilt vector}, $\vec{v}$ is some, anisotropic velocity, $(\vec{v} \odot \vec{p})_i = v_i p_i$ is the Hadamard product of the anisotropic velocity and the momentum,  and $\vec{\sigma}$ are the Pauli matrices corresponding to spin degree of freedom.
...
\end{equation}
where \( \vec{t} \) is now the (dimensionless) tilt vector.
The energy bands are~\cite{ottesenOpticalConductivityDirac2021}
\begin{equation}
  E_s(\vec{k}) = v_F \vec{t} \vec{k} \pm s v_F |k|.
\end{equation}
...
In order to more explicitly demonstrate the topological nature of the state in Eq.~(\ref{eq:3}), we will find a non-zero topological invariant associated with that state.
