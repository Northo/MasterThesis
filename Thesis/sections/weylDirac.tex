...
The dispersion of the Hamiltonian (\ref{eq:1}) has a band crossing at  $\vec{p} = 0$.
...
The Hamiltonian in Eq. (\ref{eq:1}) is not the most general, if we allow for anisotropy in the system.
...
This also illustrates that the decomposition in Eq. (\ref{eq:1}) is valid around either of the shifted nodes.
Expanding around one of the Dirac points of the Weyl semimetal, the Hamiltonian is exactly Eq. (\ref{eq:1}), after decomposing the $4 \times 4$ Hamiltonian into its two chiral  $2\times 2$ Weyl constituents.
...
The second case to consider, is a finite tilting vector $\vec{v}_0$, where we will consider only real spin, thus reducing the system back to the two-dimensional case in Eq. (\ref{eq:2}).
...
In order to more explicitly demonstrate the topological nature of the state in Eq. (\ref{eq:1}), we will find a non-zero topological invariant associated with that state.
...
\begin{equation}
  H = v_F (\vec{\sigma} + \vec{t}) \cdot \vec{p},
\end{equation}
...
Again, as the states are orthogonal, the \( \vec{t} \) terms obviously disappears.
\begin{equation}
  \begin{split}
...
\begin{equation}\label{eq:444chern}
  C = \frac{1}{2\pi} \int_C \nabla \cdot \vec{B_+} \mathrm{d} C = -1,
\end{equation}
where the property of integrals over Dirac delta distributions was used.

Note that some literature will have a Chern number differing from \eqref{eq:444chern} by the sign of the Fermi velocity,
\begin{equation}
...
The Hamiltonian Eq. (\ref{eq:4}) chosen with the opposite chirality,
\begin{equation}
