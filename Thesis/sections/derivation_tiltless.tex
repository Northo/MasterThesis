...
\todo{The reason the response is the same for the two cones, is that the expression is invariant under \( (\kappa, s) \to (-\kappa, -s)\), i.e. \( \kappa s  \) constant (i think).
This should be commented upon, as it is very important for \( t_z \) tilt
}

\todo{this argument i think is not waterthight, as we should maybe also require that kappa does not enter on its own?}
