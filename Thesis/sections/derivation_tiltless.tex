...
Using the relation of Heisenberg operators $\dot{A} = [A, H] / i $~\cite{sakuraiModernQuantumMechanics2017}, for the operator $A$ and Hamiltonian $H$, the operator
\todo{\( \frac{1}{i} \) looks weird. Is like that since we removed \( \hbar \). Replace with \( -i \)}
\begin{align}
  \vec{v} = \dot{\vec{r}} &= \frac{1}{i  } \left[ \vec{r}, H \right]\\
              &= \frac{sv_F \sigma ^i}{i  } \left[ \vec{r}, p_i + e A_i \right] + \frac{v_F}{i } [\vec{r}, \vec{t}^s \vec{k}]\\
              &= \frac{s v_F \sigma^i }{i  } \left( i \hat{\vec{x}}_i + e[\vec{r}, A_i] \right) + v_F \vec{t}^s\\
...
\begin{equation}
  \begin{split}
    ...
    \left[
      a^{\dagger}_{\vec{k} m s}(t) i \partial _0  a_{\vec{l} n s}(t)
      -
      i \left(\partial _0 a^{\dagger}_{\vec{k} ms }(t) \right) a_{\vec{l} n s}(t)
    \right]\\
...
\[
  i \partial _0 a_{\lambda }(t) = E_{\lambda }a_{\lambda },
  \quad
  i \partial _0 a^{\dagger}_{\lambda }(t) = -E_{\lambda }a^{\dagger}_{\lambda },
\]
...
\begin{comment}
  ...
  \begin{equation}
    ...
    \right].
  \end{equation}
  Now, recovering our dimensionfull quantities by letting $i\partial_{\mu }  \to v_F p_{\mu }$ \todo{what happens to s?}, which we see from comparing the QED Lagrangian in Eq. (\ref{eq:64}) to our system.
...
\begin{equation}
  \label{eq:94}
  \lim_{\omega \to 0} \lim_{\vec{q} \to 0} \chi^{xy} = \frac{e^2 v_F B}{(2 \pi)^2 \hbar},
\end{equation}
...
\begin{equation}
  \label{eq:95}
  \lim_{\omega \to 0} \lim_{\vec{q} \to 0} \chi^{xy} = \gamma_N \frac{e^2 v_F B}{(2 \pi)^2 \hbar},
\end{equation}
