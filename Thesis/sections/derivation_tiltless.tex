...
\begin{summary}
  ...
  \vspace{1em} % Fix line rbeaking of box
  \begin{align}
...
\begin{comment}
  ...
  \begin{equation}
    ...
    \right].
  \end{equation}
  Now, recovering our dimensionful quantities by letting $i\partial_{\mu }  \to v_F p_{\mu }$ \todo{what happens to s?}, which we see from comparing the \gls{qed} Lagrangian in \cref{eq:64} to our system.
...
Collecting dimensionful quantities, the response function reads
\begin{multline}
  \label{eq:90}
...
In the case of zero chemical potential, the expression may thus be simplified further, by considering only \( -N \to M = N + 1\) transitions, adding a factor of 2.
