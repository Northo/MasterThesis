...
\begin{comment}
  The stress-energy tensor of the massles \gls{qed}
  \begin{equation}
    \label{eq:64}
...
\end{equation}
As is shown in \cref{tab:transform-sign}, in the limit, \( \xi(\kappa_z) \) is odd under interchange of \( m,n \).
...
of the sum, the distribution functions \( n_{\kappa_z m s} - n_{\kappa_z n s} \) in \( \xi(\kappa_z) \) impose further restrictions on which transitions are energetically allowed.
...
The sum may be further restricted by noting that as both \( \xi(\kappa_z) \) and \( \epsilon_{\kappa_z m s} + \epsilon_{\kappa_z n s} \) are odd under \( (m, n, \kappa_z) \to (-m, -n, -\kappa_z) \), the two transitions above give the same contribution when \( \mu = 0 \).
...
\begin{figure}[ht]
  ...
  \caption{Prefactor \( \gamma_{\bar{N}} \) as a function of the number of included Landau levels~\( \bar{N} \).}
