...
We will find the current response of a single Weyl cone, with a temperature gradient $\nabla_y T$ and a magnetic field $B_z$.
The current response of interest in the given geometry is thus in the $x$-direction,
\begin{equation}\label{eq:20}
  J^x = \chi ^{xy} \left( \frac{- \nabla _yT}{T} \right),
\end{equation}
with $\chi^{xy}$  being the response function.%
\footnote{The sign in \cref{eq:20} may differ in the literature due to different sign conventions, as noted in~\cite{arjonaFingerprintsConformalAnomaly2019}. We follow that of \cite{arjonaFingerprintsConformalAnomaly2019}, differing from \cite{chernodubGenerationNernstCurrent2018}.}
This geometry is shown in \cref{fig:setup}.
\begin{figure}[ht]
  \centering
  % \includegraphics[width=0.7\textwidth]{figures/setup.png}
  \iftoggle{full-size-image}{%
    \includegraphics[width=0.7\textwidth]{figures/setup}
  }{%
    \includegraphics[width=0.7\textwidth]{figures/setup-small}
  }
...
\end{figure}
For the untilted case, the first calculation by~\textcite{chernodubGenerationNernstCurrent2018} used direct application of the scale magnetic effect~\cite{chernodubAnomalousTransportDue2016} to thermally perturbed condensed matter systems using the Luttinger formalism, where the response function
\begin{equation}
...
The first part of the calculation, however, is general for any tilt, and the specialization to \( \vec{t}_{\perp} = t_{\perp} \hat{x} \) is first made in \cref{sec:response:tilt}.
...
Fourier transforming to the frequency and momentum domain will be beneficial in our calculations.
The non-perturbed system will be taken to be time and position invariant, such that the correlator in \cref{eq:21} can be taken to depend only on the differences $t-t''$ and $\vec{r} - \vec{r}' $.
...
There is some ambiguity regarding the definition of the energy-momentum tensor~\cite{kachelriessQuantumFieldsHubble2018,chernodubThermalTransportGeometry2021,vanderwurffMagnetovorticalThermoelectricTransport2019,forgerCurrentsEnergyMomentumTensor2004}.
...
% \( S^{\alpha \mu \nu} = - S^{\mu \alpha \nu}\), the last term of \cref{eq:79} is divergence free.
...
We here show explicitly how the response differs for the two choices of the energy-momentum tensor.
...
A tilt vector parallel to the magnetic field \( \vec{t} \parallel \vec{B} \) does not alter the eigenstates, it only changes the eigenvalues by a factor \( t v_F k_z \)~\cite{yuPredictedUnusualMagnetoresponse2016,tchoumakovMagneticFieldInducedRelativisticProperties2016}, as we will show later.
