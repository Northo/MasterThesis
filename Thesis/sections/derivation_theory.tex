...
\end{equation}
with $\chi^{xy}$  being the response function\footnote{The sign in \cref{eq:20} depends on the choice of the response function being the response of the gravitational potential or the temperature gradient. Thus, the sign may differ in the literature.}.
...
\end{figure}
For the untilted case, the response was first calculated by~\textcite{chernodubGenerationNernstCurrent2018} by direct application of the scale magnetic effect~\cite{chernodubAnomalousTransportDue2016} to thermally perturbed condensed matter systems using the Luttinger formalism, where the response function
\begin{equation}
...
A tilt vector parallel to the magnetic field \( \vec{t} \parallel \vec{B} \) does not alter the eigenstates, it only changes the eigenvalues by a factor \( t v_F k_z \)~\cites{yuPredictedUnusualMagnetoresponse2016,tchoumakovMagneticFieldInducedRelativisticProperties2016}, as we will show later.
