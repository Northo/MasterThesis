...
\todo{Maybe better to introduce some quanity here, for example \(\chi\) representing resized y}
\begin{equation}
  \label{eq:40}
  \tilde{\phi} _{\vec{k} m s} (\vec{r})
  =
  \frac{1}{\sqrt{L_xL_z}}
  \frac{e^{ik_x x}e^{i k_z z}}{\sqrt{\alpha _{\tilde{k}_z m s}^2 + 1}}
  e^{-\frac{1}{2} \chi ^2}
  \begin{pmatrix}
    \frac{\alpha _{\tilde{q}_z m s}}{\sqrt{2^{M-1} (M-1)! \sqrt{\pi } l_{B'}}} H_{M-1}\left( \chi  \right)\\
...
\end{equation}
A straightforward but tedious calculation shows that this can be written as
\begin{equation}
  \label{eq:47}
...
\begin{equation}
  ...
  \begin{split}
   ...
   &\left[
    ...
    H_{M-1} \left(  \tilde{y} + \frac{l_{B}}{2 \sqrt{\alpha }} \left(\alpha (B-A) - i q_{y}\right) \right)
    H_N\left( \tilde{y} + \frac{l_{B}}{2 \sqrt{\alpha }} \left(\alpha (A-B) - i q_{y}\right) \right) \right.\\
    &- t_{x} a_{\vec{k}ms}a_{\vec{k} + \qvec{q} ns}
    H_{M-1} \left( \tilde{y} + \frac{l_{B}}{2 \sqrt{\alpha }} \left(\alpha (B-A) - i q_{y}\right) \right)
    H_{N-1}\left( \tilde{y} + \frac{l_{B}}{2 \sqrt{\alpha }} \left(\alpha (A-B) - i q_{y}\right) \right)\\
   & +
    b_{\vec{k}ms} a_{\vec{k} + \qvec{q} ns}
    H_M \left( \tilde{y} + \frac{l_{B}}{2 \sqrt{\alpha }} \left(\alpha (B-A) - i q_{y}\right) \right)
    H_{N-1} \left( \tilde{y} + \frac{l_{B}}{2 \sqrt{\alpha }} \left(\alpha (A-B) - i q_{y}\right) \right)\\
    &\left. - t_{x}
    b_{\vec{k}ms} b_{\vec{k} + \qvec{q} ns}
    H_M \left( \tilde{y} + \frac{l_{B}}{2 \sqrt{\alpha }} \left(\alpha (B-A) - i q_{y}\right) \right)
    H_{N} \left(  \tilde{y} + \frac{l_{B}}{2 \sqrt{\alpha }} \left(\alpha (A-B) - i q_{y}\right)\right)
    \right].
  \end{split}
\end{equation}
\begin{align}
...
\begin{multline}
  ...
  H_{M-1} \left( \tilde{y} + \frac{l_B}{2 \sqrt{\alpha }}(\alpha (B-A) - iq_y) \right)
  H_{N-1}\left( \tilde{y} + \frac{l_B}{2 \sqrt{\alpha }}(\alpha (A-B) - iq_y) \right)
  ,
\end{multline}
and
\begin{multline}
  ...
  \int \mathrm{d}\tilde{y} \; l_B e^{-y^2}
  b_{\vec{k}ms}b_{\vec{k} + \qvec{q} ns}\\
  H_{M} \left( \tilde{y} + \frac{l_B}{2 \sqrt{\alpha }}(\alpha (B-A) - iq_y) \right)
  H_{N}\left( \tilde{y} + \frac{l_B}{2 \sqrt{\alpha }}(\alpha (A-B) - iq_y) \right)
  ,
\end{multline}
with
\begin{align}
  \Xi_3 ^{(1)}(\vec{q}, m, n, s) &= \alpha _{\vec{k} m s} \alpha _{\vec{k} + \qvec{q} n s} \sqrt{\frac{2^{N-1} (M-1)!}{2^{M-1} (N-1)!}}
                                   \left( \frac{\alpha (A-B) - iq_y}{2 \sqrt{\alpha }} l_B \right)^{N-M}
                                   L^{N-M}_{M-1} (\xi ),\\
                                   %%%
  \Xi_3 ^{(2)}(\vec{q}, m, n, s) &= \alpha _{\vec{k} m s} \alpha _{\vec{k} + \qvec{q} n s} \sqrt{\frac{2^{M-1} (N-1)!}{2^{N-1} (M-1)!}}
                                   \left( \frac{\alpha (B-A) - iq_y}{2 \sqrt{\alpha }} l_B \right)^{M-N}
                                   L^{M - N}_{N-1} (\xi),\\
  \Xi_3(\vec{q}, m, n, s) &=
          \begin{cases}
            \Xi _3 ^{(1)} & \text{if } N \geq M\\
            \Xi _3 ^{(2)} & \text{if } N \leq M
...
with
\[
  \xi = \frac{l_{B}^2}{2 \alpha } \left[q_{y}^2 + \alpha^2 (B-A)^2\right].
\]

Expressed in these quantites, we may write (corresponding to Eq. (4.74))
\begin{multline}
  \label{eq:32}
  ...
  \left[
    \chi_{1} (\vec{q}, m, n, s)
    + \chi _{2} (\vec{q}, m, n, s)
    - t_{x} \chi _{3} (\vec{q}, m, n, s)
    - t_{x} \chi _{4} (\vec{q}, m, n, s)
  \right].
\end{multline}
...
\begin{multline}
  \label{eq:39}
  J_{\vec{k} m s; \vec{k} + \qvec{q} n s}^{(2)} (\vec{q}) =
  \frac{e \omega _{0 x} l_{B}}{\mathcal{N}^{*} \mathcal{N}}
  \int \mathrm{d} \tilde{y}
    \exp \left[
      -\frac{l_{B}^2}{4 \alpha } (q_{y}^2 + 2i (A+B) q_{y} \alpha + (A-B)^2 \alpha^2)
    \right]\\
  e^{-\tilde{y}^2} \big[
    a_{\vec{k} m s} H_{M-1}(\dots) \left(a_{\vec{k} + \qvec{q} n s} H_{N-1} - t_{x} b_{\vec{k} + \qvec{q} n s} H_{N} \right)\\
   +
    b_{\vec{k} m s} H_{M}(\dots) \left(-t_{x} a_{\vec{k} + \qvec{q} n s} H_{N-1} + b_{\vec{k} + \qvec{q} n s} H_{N} \right)
    \big]
    (1 - t_{x}^2)^{-\frac{1}{2}}.
\end{multline}
Thus
...
\begin{multline}
 ...
 \Big[
  ...
  H_{N-1} \left(\tilde{y} + \frac{l_{B}}{2 \sqrt{\alpha }} (\alpha(B-A) + iq_{y})\right)
  H_{M} \left(
  \tilde{y} + \frac{l_{B}}{2 \sqrt{\alpha}} \left( \alpha (A-B) + i q_{y} \right)
\right  ) \\
  + b_{\vec{k} + \qvec{q} ns } a_{\vec{k} m s}
  H_{N} \left(
  \tilde{y} + \frac{l_{B}}{2 \sqrt{\alpha }} (\alpha(B-A) + iq_{y})
\right  )
  H_{M-1} \left(
  \tilde{y} + \frac{l_{B}}{2 \sqrt{\alpha}} \left( \alpha (A-B) + i q_{y} \right)
\right  )
...
\begin{multline}
  ...
  \phi _{\vec{k} ms} (y)\\
  %
  = -\frac{i\hbar 2}{\alpha \sqrt{1 - t_{x}^2} }
  e^{-\frac{1}{2} \chi _{\vec{k}}^2 - \frac{1}{2} \chi _{\vec{k} + \qvec{q}} ^2}\\
  \Big[
...
\begin{multline}
  ...
  \phi_{\vec{k} ms} (y)
  =
  - \frac{i\hbar}{\alpha 2 \sqrt{1 - t_{x}^2} }
  \exp
  \left[
  ...
  \Big[
  a_{\vec{k} + \qvec{q} ns} H_{N-1}(\tilde{y} + \frac{l_{B}}{2 \sqrt{\alpha }} \left(\alpha (A-B) + i q_{y}\right))
  \left\{
    a_{\vec{k} ms} (M-1) H_{M-2} (\tilde{y} + \frac{l_{B}}{2 \sqrt{\alpha }} \left(\alpha (B-A) + i q_{y}\right))
    - t_{x} b_{\vec{k} ms} M H_{M-1} (\tilde{y} + \frac{l_{B}}{2 \sqrt{\alpha }} \left(\alpha (B-A) + i q_{y}\right))\right\}\\
  +
  b_{\vec{k} + \qvec{q} ns} H_{N} (\tilde{y} + \frac{l_{B}}{2 \sqrt{\alpha }} \left(\alpha (A-B) + i q_{y}\right))
  \left\{
    -t_{x} a_{\vec{k} ms} (M-1) H_{M-2} (\tilde{y} + \frac{l_{B}}{2 \sqrt{\alpha }} \left(\alpha (B-A) + i q_{y}\right))
    + b_{\vec{k} ms} M H_{M-1} (\tilde{y} + \frac{l_{B}}{2 \sqrt{\alpha }} \left(\alpha (B-A) + i q_{y}\right))
  \right\}
  \Big].
\end{multline}
\todo{probably better to go back to using \(\chi \), and then include here a refresher of what that is expressed with \(\tilde{y}\)}
...
p^{\mu } = (\frac{E}{c}, \vec{p}),
\]
Eq. \eqref{eq:44} is nothing more than
\begin{equation}
  \label{eq:51}
  H^2-E^2 = v_{F}^2 \vec{p}^2 - c^2p^{(0) 2},
\end{equation}
