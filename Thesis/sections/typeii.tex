...
\begin{multline}
  ...
  \left[
    \Xi_{1} (\vec{q}, m, n, s)
    + \Xi _{2} (\vec{q}, m, n, s)
    - t_{x} \Xi _{3} (\vec{q}, m, n, s)
    - t_{x} \Xi _{4} (\vec{q}, m, n, s)
  \right].
\end{multline}
...
\begin{equation}
  ...
  F_{21}
  =
    -t_{x} (M-1) \frac{a_{\vec{k} m s}}{l_B a_{\vec{k} m \mp 2 s}} \frac{\Xi_{1} (\vec{q}, m\mp 2, n, s)}{
    \sqrt{\alpha _{\vec{k} m\mp 2 s} ^2 + 1}
    \sqrt{\alpha _{\vec{k} + \qvec{q} n s}^2 + 1}
    },
\end{equation}
and
\begin{equation}
  F_{12}
  = -t_{x} M \frac{a_{\vec{k} + \qvec{q} n s}}{l_B a_{\vec{k} + \qvec{q} n \mp 1 s}}
  \frac{\Xi_{1}(\vec{q}, m, n\mp1, s)}{
    \sqrt{\alpha _{\vec{k} m s}^2 + 1}
    \sqrt{\alpha _{\vec{k} + \qvec{q} n \mp 1 s} ^2 + 1}
  }.
\end{equation}
\( F_{11} \) and \( F_{22} \) are the same as for the untilted case:
\begin{equation}
  \label{eq:56}
  F_{11} = \frac{\alpha_{\vec{k} m s} \alpha_{\vec{k} + \qvec{q} ns} \sqrt{M-1} }{ l_B \sqrt{2} \alpha _{\vec{k} m\mp 1 s}}
  \frac{\Xi_{1} (\bar{\vec{q}}, m\mp 1, n\mp 1, s)}{
    \sqrt{\alpha _{\vec{k} m s}^2 + 1}
    \sqrt{\alpha _{\vec{k} + \qvec{q} n s} ^2 + 1}
  },
\end{equation}
...
\begin{align}
  \label{eq:58}
  T^{0y~(2)}_{\vec{k} + \qvec{q} ns, \vec{k} ms} (\vec{q}) &= + \frac{v_F}{2} \int \mathrm{d} y
  e^{i q_y q} \phi _{\vec{k} + \qvec{q} ns}^{*} (y) p_{y} \phi _{\vec{k} ms} (y)\\
&= -\frac{ i \hbar v_{F} }{2}
                                                                                     \Gamma _{\vec{k} \qvec{q} m n s}^+
\sum\limits_{i,j}^{} F_{ij},
\end{align}
where
\[
  \Gamma _{\vec{k} \qvec{q} m n s}^+ =
