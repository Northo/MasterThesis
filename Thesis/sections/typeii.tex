...
\begin{equation}
  \label{eq:4}
  H = \vec{\omega_{0}} \vec{k} + s (\vec{v} \odot \vec{k}) \vec{\sigma},
\end{equation}
...
\begin{equation}
  \label{eq:5}
  H_{B} = \omega _{\perp} \left(k_{x} - e B_{z} y \right) + \omega _{\parallel} k_{z} + s v_{y} k_{y} \sigma _{y} + s v_{z} k_{z} \sigma _{z} + s v_{x} \left(k_{x} - e B_{z} y\right) \sigma _{x},
\end{equation}
...
\begin{equation}
  \label{eq:8}
  H_{B} = \left(\omega _{\perp} k^B_{x} + \omega _{\parallel} k^B_{z} \right) \mathcal{I}_2 + \sum_i s v_{i} k^B_{i} \sigma _{i},
\end{equation}
...
\[
  \omega _{\perp} k^B_{x} \mathcal{I}_{2} + s v_{x} k^B_{x} \sigma _{x}.
\]
...
\begin{equation}
  \label{eq:11}
  \omega _{\perp} \cosh \theta + s v_{x} \sinh \theta = 0.
\end{equation}
Solving for \(\theta \) we get
\begin{equation}
  \label{eq:12}
  \theta = \log (
  \pm \frac{\sqrt{s v_{x} - \omega _{\perp}}}{\sqrt{s v_{x} + \omega _{\perp}}}
  ).
\end{equation}
...
\begin{multline}
  J^x_{\vec{k} m s, \vec{k} + \qvec{q} ns} (\vec{q}) T^{0y~(i)}_{\vec{k}+\qvec{q} n s, \vec{k}ms}(\vec{q})
  =
  \frac{v_F e i s \alpha}{4}
  e^{-P^2}
  \frac{
    (E_{\vec{k} m s} + E_{\vec{k}+\qvec{q} n s})
    (\alpha_{\vec{k} m s} \Xi_1 (m, n) - \alpha_{\vec{k} + \qvec{q} n s} \Xi_2(m, n))
  }{
    (\alpha_{\vec{k} m s}^2 + 1)(\alpha_{\vec{k}+\qvec{q} n s}^2 + 1)
  }\\
  \left[(s-t_x^2)(\alpha_{\vec{k} m s} \Xi_1(m,n) + \alpha_{\vec{k} + \qvec{q} n s} \Xi_2(m,n)) + t_x (1-s) ( \alpha_{\vec{k} m s} \alpha_{\vec{k} + \qvec{q} n s} \Xi_1(m, n\mp 1) + \Xi_2(m, n \pm 1) )\right]
  \label{eq:75}
\end{multline}
...
\begin{multline}
  \label{eq:76}
  \lim_{\omega \to 0} \lim_{\vec{q} \to 0} \chi^{xy}(\omega, \vec{q}) = \lim_{\eta \to 0}
  \frac{- e s \alpha v_F \sqrt{e B} }{4 (2 \pi)^2 \sqrt{2} }
  \sum\limits_{mn}^{}
  \int \mathrm{d}k_z
  e^{-P^2}\\
  \frac{
  ...
  \big[
    ...
    + t_x (1-s) (
    \alpha_{\vec{k} m s} \alpha_{\vec{k} + \qvec{q} n s} \Xi_1(m, n\mp 1) + \Xi_2(m, n \pm 1)
    )\big]
\end{multline}
...
In the fourth quadrant, \( m \geq 0, n \leq 0 \), the step function factor may be simplified.
...
Consider the part of the response function from the product \( J^{(1)} T^{(1)} \)
\todo{Take more care in the expressions with being careful and clear on whether we have taken the limit \( q\to 0 \) or not}
\begin{equation}
  \label{eq:64}
