...
\begin{equation}
  \label{eq:40}
  H = v_{F} \vec{t} \vec{k} + s (\vec{v} \odot \vec{k}) \vec{\sigma},
\end{equation}
where \(\vec{t}\) is the \emph{tilt vector} and \(\vec{v}\) is the Fermi velocity, which in general is anisotropic.
...
\begin{equation}
  \label{eq:41}
  H_{B} = v_F t _{\perp} \left(k_{x} - e B_{z} y \right) + v_F t _{\parallel} k_{z} + s v_{y} k_{y} \sigma _{y} + s v_{z} k_{z} \sigma _{z} + s v_{x} \left(k_{x} - e B_{z} y\right) \sigma _{x},
\end{equation}
...
Act with the hyperbolic rotation operator \(\exp[\Theta /2 \sigma_{x}]\) on Eq. \eqref{eq:42}, and insert identity on the form \(\exp[\Theta /2 \sigma_{x}]\exp[-\Theta /2 \sigma_{x}]\) before the state vector.
...
We rewrite the Hamiltonian (Eq. \eqref{eq:41}) in a more compact and explicit way
\begin{equation}
  \label{eq:44}
  H_{B} = v_F \left(t _{\perp} k^B_{x} + t _{\parallel} k^B_{z} \right) \mathcal{I}_2 + \sum_i s v_{i} k^B_{i} \sigma _{i},
\end{equation}
...
\[
  v_F t_{\perp} k^B_{x} \mathcal{I}_{2} + s v_{x} k^B_{x} \sigma _{x}.
\]
...
Visually, this can be visualized by plotting the \(t\)-vector inside a unit sphere, shown in Figure \ref{fig:tiltSphere}.
...
\begin{multline}
  ...
  (\exp[\Theta /2 \sigma_{x}] H_{B}\exp[\Theta /2 \sigma_{x}] - E \exp[\Theta \sigma_{x}]) \ket{\tilde{\psi}} = 0\\
  = v_F \begin{pmatrix}
    k_z (1 + t_{\parallel} \gamma) - \frac{E \gamma}{v_{F}}   & -i k_{y} + k_{x} / \gamma  - \frac{E  \gamma \beta}{v_{F}}  + k_{z} t_{\parallel} \gamma \beta \\
      i k_{y} + k_{x} / \gamma  - \frac{E  \gamma \beta}{v_{F}}  + k_{z} t_{\parallel} \gamma \beta
    & -k_{z} (1 - t_{\parallel} \gamma) - \frac{E \gamma}{v_{F}}
    \end{pmatrix}
    \ket{\psi}.
\end{multline}
...
\begin{equation}
  \begin{split}
    \tilde{k}_{x} &= k_{x} / \gamma + \gamma \beta (k_{z} t_{\parallel} - E /v_F),\\
    \tilde{k}_{y} &=  k_{y},\\
    \tilde{k}_{z} &=  k_{z}.\\
  \end{split}
\end{equation}
...
\[
  \tilde{q}_{x} = \alpha q_{x} - \frac{\beta}{\alpha} \frac{E_{k_z m s}- k_{z} v_F t_{\parallel} }{v_{x}}
  = \alpha q_x - \beta \frac{E^0_{m, \alpha B} }{v_{F}}
\]
...
\end{equation}
or if we symmetrize in daggered and undaggered fields
...
The energy-momentum tensor of the Dirac Lagrangian is \cite[Eq.~3-153]{itzyksonQuantumFieldTheory1980}
\begin{equation}
  \label{eq:21}
  T^{\mu \nu} = \partial^{\nu} \bar{\psi} \frac{\delta \mathcal{L}}{\delta (\partial_{\mu} \bar{\psi})} + \frac{\delta \mathcal{L}}{\delta (\partial_{\mu} \psi)} \partial^{\nu} \psi - \eta^{\mu \nu} \mathcal{L}.
\end{equation}

\todo{The following section is almost correct, but the ordering is wrong in the psi bar case.}
The \emph{canonical} energy-momentum tensor is defined by
\begin{equation}
  \label{eq:19}
  T^{\mu \nu} =  \frac{\delta \mathcal{L}}{\delta(\partial_{\mu} \phi_{i})} \partial_{\nu} \phi_i - \eta^{\mu \nu} \mathcal{L},
\end{equation}
where the index \( i \) runs over the types of fields.
For our Lagrangian we thus have
\begin{equation}
  \label{eq:20}
  T^{\mu \nu} =
  \frac{i}{2} (
