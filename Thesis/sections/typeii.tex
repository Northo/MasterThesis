...
We now return to solving Eq. \eqref{eq:38}, using the solution angle we just found.
...
By naively employing Eq. \eqref{eq:62}, the resulting canonical energy-momentum tensor of the Dirac theory would be
\begin{equation}
...
Adapting the definition Eq. \eqref{eq:64} the energy-momentum tensor for the untilted Dirac cone is thus
\begin{equation}
  \label{eq:66}
  T^{\mu \nu} =
  \frac{i}{2} (
...
Due to the boost transformation, the elements of the spinor in the untilted system, Eq. \eqref{eq:54}, mix.
...
Eq. \eqref{eq:76} is on the same for as in the untilted cone case, and we may thus proceed with the same method.
...
Recalling the expression for \(\tilde{\phi}\) from Eq. \eqref{eq:72},
\[
  \tilde{\phi} = e^{-\frac{1}{2} \chi ^2}
  \begin{pmatrix}
    a_{\vec{k} m s} H_{M-1} (\chi)\\
...
The exponent is the same as that which we found for the current operator case, Eq. \eqref{eq:76}, with the change \(q_{y} \to - q_{y}\).
...
Simply evaluating Eq. \eqref{eq:105}, yields
\begin{equation}
  \label{eq:106}
...
\]
the operator in Eq. \eqref{eq:108} is nothing more than
...
Using now the effective speed of light \( c=v_F \), Eq. \eqref{eq:109} is
\begin{equation}
  \label{eq:110}
  - v_F^2 p_{\mu } p^{\mu }.
\end{equation}
